...
| `mcp-server/test-structural-claims.mjs` | a subsystem advances to `structural` with no current `<sid>/` claim; a claim closed by `apply_change_impact` still appears as current in `describe_locus` or on the page; a subsystem with zero claims renders an empty structure section instead of the labelled fallback | claim presence does not prove claim truth; that stays the adversarial pass's job |
