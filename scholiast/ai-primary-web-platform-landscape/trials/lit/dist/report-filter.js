...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // lit/src/report-filter.js
  var ReportFilter = class extends i4 {
    constructor() {
      super();
      this.query = "";
    }
    render() {
      ...
      return b2`<label>Filter candidates <input type="search" @input=${(event) => {
        this.query = event.target.value;
      }}></label>
      <table><caption>Candidate evidence</caption><thead><tr><th scope="col">Candidate</th><th scope="col">Disposition</th></tr></thead>
      <tbody>${rows.map((row) => b2`<tr><th scope="row">${row[0]}</th><td>${row[1]}</td></tr>`)}</tbody></table>`;
    }
  };
  __publicField(ReportFilter, "properties", { query: { state: true } });
  __publicField(ReportFilter, "styles", i`:host{display:block} label{display:block;margin:1rem 0} input{font:inherit;padding:.4rem}`);
