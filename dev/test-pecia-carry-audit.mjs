...
const PRODUCER_REL = "dev/amanuensis-defects-to-pecia.mjs";
...
const HEADLINES = {
  ...
  producer:
    "the accounting the document cites its generator for is not the accounting that generator emits",
...
if (live?.unreadable) {
  ...
  // The producer arm. `dev/pecia-dogfood.md` names
  // `--carry-audit --markdown` as what regenerates the table above, and until
  // this arm existed nothing ran it: the whole `if (carryAudit)` branch of the
  // generator could be deleted and PA1 stayed green, because every assertion
  // re-derived the accounting from the store directly rather than from the
  // tool the document cites as its source. The generator is the thing under
  // test here and the committed table is the expectation it must reproduce,
  // which is the only arrangement in which the document's own regeneration
  // instruction is a checked claim rather than a sentence.
  const producer = join(REPO, PRODUCER_REL);
  checked += 1;
  if (!existsSync(producer)) {
    record("producer", `${PRODUCER_REL} is absent, so the accounting cannot be regenerated`);
  } else {
    const run = spawnSync(process.execPath, [producer, "--carry-audit", "--markdown"], {
      cwd: REPO,
      encoding: "utf8",
    });
    if (run.error) {
      record("producer", `${PRODUCER_REL} --carry-audit --markdown could not be run`);
    } else if (run.status !== 0) {
      record(
        "producer",
        `${PRODUCER_REL} --carry-audit --markdown exits ${run.status} against the live store, ` +
          `so ${DOGFOOD_REL} cannot be regenerated by the command it names`,
      );
    } else {
      const generated = parseAccounting(String(run.stdout ?? ""));
      if (generated === null || generated.ambiguous || generated.rows.length === 0) {
        record(
          "producer",
          `${PRODUCER_REL} --carry-audit --markdown emits no accounting table whose first ` +
            `column is "${TABLE_KEY}", so the command ${DOGFOOD_REL} names no longer regenerates it`,
        );
      } else {
        const key = (row) =>
          `${row.pecia_id} ${row.reference} carried_id ${row.carried_id} ${row.outcome}`;
        const emitted = new Map(generated.rows.map((row) => [row.pecia_id, key(row)]));
        const committed = new Map((table?.rows ?? []).map((row) => [row.pecia_id, key(row)]));
        for (const [peciaId, line] of committed) {
          checked += 1;
          if (!emitted.has(peciaId)) {
            record(
              "producer",
              `${DOGFOOD_REL} accounts for ${peciaId} and the regenerated accounting does not ` +
                `report it at all`,
            );
          } else if (emitted.get(peciaId) !== line) {
            record(
              "producer",
              `${DOGFOOD_REL} records "${line}" for ${peciaId} and the regenerated accounting ` +
                `records "${emitted.get(peciaId)}"`,
            );
          }
        }
        for (const peciaId of emitted.keys()) {
          checked += 1;
          if (!committed.has(peciaId)) {
            record(
              "producer",
              `the regenerated accounting reports ${peciaId} and ${DOGFOOD_REL} does not ` +
                `account for it`,
            );
          }
        }
